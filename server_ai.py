...
AI_FRIEND_ID = 99999
AI_FRIEND_NAME = "🤖 AI助手"
AI_API_KEY = "5905767dc3864569b9609e75a60a7fe0.yhmHLVBDxCOO47AR"
...
try:
    ...
    import zai
    AI_CLIENT = ZhipuAiClient(api_key=AI_API_KEY, base_url=AI_API_BASE_URL)
    AI_ENABLED = True
    print("✅ Zai SDK 导入成功，AI功能已启用")
except ImportError as e:
    print(f"⚠️ Zai SDK 未安装，使用模拟模式: {e}")
    AI_ENABLED = False
...
def get_ai_response(user_id, message):
    ...
    try:
        print(f"开始调用AI API - user_id: {user_id}")
        ...
        print(f"AI API调用成功 - reply: {ai_reply[:50]}...")
        
        return ai_reply
        
    except zai.core.APIStatusError as err:
        print(f"AI API状态错误: {err}")
        return f"AI服务暂时不可用: {err}"
...
def get_mock_response(message):
    """模拟AI回复（当Zai SDK未安装时使用）"""
    lower_content = message.lower()
    
    if "你好" in lower_content or "hello" in lower_content:
        return "你好！我是你的AI助手，很高兴为你服务！😊"
    elif "天气" in message:
        return "今天天气晴朗，温度适宜，非常适合外出活动！☀️"
    elif "时间" in message:
        return f"现在的时间是：{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    elif "谢谢" in message or "thank" in lower_content:
        return "不用谢！有任何问题随时找我哦！😄"
    elif "笑话" in message:
        return "为什么程序员总是分不清圣诞节和万圣节？因为 Dec 25 等于 Oct 31！😆"
    elif "帮助" in message or "help" in lower_content:
        return "我可以帮你解答问题、聊天、讲笑话，或者提供各种信息。有什么需要帮助的吗？"
    else:
        return f"你说的是：\"{message}\"，我收到了！这是一个很好的话题，我们可以继续聊聊~"
...
def init_db():
    ...
    # 添加AI用户（如果不存在）
    cursor.execute('SELECT id FROM users WHERE id = %s', (AI_FRIEND_ID,))
    if not cursor.fetchone():
        # AI用户不需要密码，密码字段填充默认值
        cursor.execute('INSERT INTO users (id, username, password) VALUES (%s, %s, %s)',
                       (AI_FRIEND_ID, AI_FRIEND_NAME, 'AI_USER_NO_PASSWORD'))
        print(f"✅ AI用户已添加到数据库 - ID: {AI_FRIEND_ID}, Name: {AI_FRIEND_NAME}")
    else:
        print(f"✅ AI用户已存在 - ID: {AI_FRIEND_ID}")
...
def get_friends(user_id):
    ...
    # 在好友列表顶部动态添加AI助手（无需修改数据库）
    ai_friend = {
        'id': AI_FRIEND_ID,
        'username': AI_FRIEND_NAME,
        'is_system': True
    }
    result.insert(0, ai_friend)
...
class ChatWebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def handle_ai_message(self, user_id, content):
        """处理AI消息并生成回复"""
        try:
            print(f"开始处理AI消息 - user_id: {user_id}, content: {content[:50]}")
            io_loop = tornado.ioloop.IOLoop.current()  # 获取主事件循环的引用
            
            # 在后台线程中调用AI，避免阻塞事件循环
            def generate_response():
                try:
                    print(f"线程开始生成AI回复 - user_id: {user_id}")
                    ai_reply = get_ai_response(user_id, content)
                    print(f"AI回复生成完成 - user_id: {user_id}, reply: {ai_reply[:50]}...")
                    
                    # 保存AI回复消息到数据库
                    ai_message_id = save_message(AI_FRIEND_ID, user_id, ai_reply)
                    print(f"AI回复已保存 - message_id: {ai_message_id}")
                    
                    ai_message_data = {
                        'id': ai_message_id,
                        'from_user_id': AI_FRIEND_ID,
                        'to_user_id': user_id,
                        'content': ai_reply,
                    ...
                    io_loop.add_callback(self.send_ai_response, user_id, ai_message_data)
                    print(f"AI回复回调已注册到主事件循环")
            ...
            # 在线程池中执行AI调用
            print(f"启动线程执行AI调用")
            tornado.ioloop.IOLoop.current().run_in_executor(db_executor, generate_response)
            print(f"线程已启动，等待结果")
            
        except Exception as e:
            print(f"处理AI消息错误: {e}")
    
    def send_ai_response(self, user_id, message_data):
        """发送AI回复消息"""
        try:
            with online_users_lock:
                if user_id in online_users:
                    target_ws = online_users[user_id]
                    target_ws.write_message(json.dumps({
                        'type': 'new_message',
                        'data': message_data
                    }))
                    print(f"AI回复已推送给用户 {user_id}")
                else:
                    # 保存离线消息
                    save_offline_message(AI_FRIEND_ID, user_id, message_data['content'])
                    print(f"用户 {user_id} 离线，AI回复已存储到离线消息表")
        except Exception as e:
            print(f"发送AI回复错误: {e}")
